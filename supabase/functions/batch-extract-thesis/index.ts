  ...
  try {
    ...
    if (action === 'check') {
      ...
      const [contactsResult, thesesResult] = await Promise.all([
        ...
        supabase.from('theses').select('id', {
          count: 'exact',
          head: true
        })
      ]);
    ...
    const { data: existingTheses } = await supabase.from('theses').select('contact_id');
    const existingContactIds = new Set((existingTheses || []).map((t)=>t.contact_id));
    // Get contacts that need extraction
    const { data: contacts, error: contactsError } = await supabase.from('contacts').select('id, name, bio, title, company, location, investor_notes').or('bio.not.is.null,title.not.is.null,investor_notes.not.is.null').limit(batchSize * 2); // Fetch more to filter
    if (contactsError) throw contactsError;
    // Filter to contacts without thesis
    const contactsToProcess = (contacts || []).filter((c)=>!existingContactIds.has(c.id)).slice(0, batchSize);
    ...
    for(let i = 0; i < contactsToProcess.length; i += PARALLEL_SIZE){
      ...
      const results = await Promise.allSettled(batch.map(async (contact)=>{
        try {
          ...
          // Save to database
          const { error: insertError } = await supabase.from('theses').upsert({
    ...
    const { count: remainingCount } = await supabase.from('contacts').select('id', {
      count: 'exact',
      head: true
    }).or('bio.not.is.null,title.not.is.null,investor_notes.not.is.null').not('id', 'in', `(${[
      ...existingContactIds,
      ...contactsToProcess.map((c)=>c.id)
    ].join(',')})`);
    return new Response(JSON.stringify({
...
async function extractThesisForContact(contact, openaiKey) {
  const profileText = [
