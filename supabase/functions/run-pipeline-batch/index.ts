...
async function processPipeline(supabase, pipeline, openaiKey, startTime) {
...
async function checkForMoreWork(supabase, userId) {
  const { count: needsEnrich } = await supabase.from("contacts").select("*", {
    count: "exact",
    head: true
  }).eq("owned_by_profile", userId).not("name", "is", null).is("bio", null);
  const { count: needsThesis } = await supabase.from("contacts").select("*", {
    count: "exact",
    head: true
  }).eq("owned_by_profile", userId).or("bio.not.is.null,title.not.is.null,investor_notes.not.is.null").is("thesis_sectors", null).is("thesis_stages", null);
  const { count: needsEmbed } = await supabase.from("contacts").select("*", {
    count: "exact",
    head: true
  }).eq("owned_by_profile", userId).not("bio", "is", null).is("bio_embedding", null);
  return (needsEnrich || 0) > 0 || (needsThesis || 0) > 0 || (needsEmbed || 0) > 0;
}
async function processEnrichmentBatch(supabase, pipeline, openaiKey, startTime) {
  const userId = pipeline.owned_by_profile;
  const { data: contacts, error } = await supabase.from("contacts").select("id, name, company, title, bio").eq("owned_by_profile", userId).not("name", "is", null).is("bio", null).order("id").limit(BATCH_SIZE);
...
async function processExtractionBatch(supabase, pipeline, openaiKey, startTime) {
  const userId = pipeline.owned_by_profile;
  const { data: contacts, error } = await supabase.from("contacts").select("id, name, bio, title, investor_notes").eq("owned_by_profile", userId).not("bio", "is", null).order("id").limit(BATCH_SIZE * 2);
  if (error) throw error;
  if (!contacts || contacts.length === 0) {
    return {
      processed: 0,
  ...
  const { data: existingTheses } = await supabase.from("theses").select("contact_id").in("contact_id", contactIds);
  const thesisIds = new Set((existingTheses || []).map((t)=>t.contact_id));
  const needsThesis = contacts.filter((c)=>!thesisIds.has(c.id)).slice(0, BATCH_SIZE);
...
async function processEmbeddingBatch(supabase, pipeline, openaiKey, startTime) {
  const userId = pipeline.owned_by_profile;
  const { data: contacts, error } = await supabase.from("contacts").select("id, name, bio, title, investor_notes").eq("owned_by_profile", userId).not("bio", "is", null).is("bio_embedding", null).order("id").limit(BATCH_SIZE);
  if (error) throw error;
  if (!contacts || contacts.length === 0) {
    return {
      processed: 0,
...
async function generateBio(contact, apiKey) {
...
async function extractThesis(contact, apiKey) {
  const content = [
    contact.bio,
    contact.title,
    contact.investor_notes
