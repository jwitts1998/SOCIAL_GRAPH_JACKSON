...
Return JSON:
{
  ...
  "summary": "One sentence investment thesis summary"
    ...
    const { data: existingThesis } = await supabaseService.from('theses').select('id').eq('contact_id', contactId).single();
    ...
    if (existingThesis) {
      ...
      const { data, error } = await supabaseService.from('theses').update(thesisData).eq('id', existingThesis.id).select().single();
      if (error) {
      ...
      // Insert new thesis
      const { data, error } = await supabaseService.from('theses').insert(thesisData).select().single();
      if (error) {
