...
Deno.serve(async (req)=>{
  ...
  try {
    ...
    if (mode === 'batch') {
      ...
      const { data: contacts, error: fetchError } = await supabaseService.from('contacts').select('id, name, bio, investor_notes, title, company').eq('owned_by_profile', user.id).is('bio_embedding', null).limit(50); // Process 50 at a time
      if (fetchError) {
        throw fetchError;
      }
      if (!contacts || contacts.length === 0) {
        return new Response(JSON.stringify({
          message: 'No contacts need embeddings',
          processed: 0
        }), {
          headers: {
            ...corsHeaders,
      ...
      for (const contact of contacts){
        try {
          ...
          if (bioEmbedding || thesisEmbedding) {
            ...
            const { error: updateError } = await supabaseService.from('contacts').update(updateData).eq('id', contact.id);
            if (updateError) {
    ...
    const { data: contact, error: fetchError } = await supabaseService.from('contacts').select('id, name, bio, investor_notes, title, company, owned_by_profile').eq('id', contactId).single();
    ...
    if (!bioEmbedding && !thesisEmbedding) {
      return new Response(JSON.stringify({
        message: 'No text available to generate embeddings'
      }), {
        headers: {
          ...corsHeaders,
          'Content-Type': 'application/json'
    ...
    if (bioEmbedding) updateData.bio_embedding = JSON.stringify(bioEmbedding);
    if (thesisEmbedding) updateData.thesis_embedding = JSON.stringify(thesisEmbedding);
    const { error: updateError } = await supabaseService.from('contacts').update(updateData).eq('id', contactId);
    if (updateError) {
      throw updateError;
    }
